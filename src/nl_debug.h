...
#define trace_event_cow_data(_type, _bio, _priv, _data, _size)	\
({ 												\
	struct params_t params = { 0 };			\
											\
	if (_bio) { 							\
	...
	params.priv1 = (_priv); 					\
	params.priv2 = 0; 					\
	nl_send_data(_type, __func__, __LINE__, &params, _data, _size); \
})

int nl_send_event(enum msg_type_t type, const char *func, int line, struct params_t *params);
int nl_send_data(enum msg_type_t type, const char *func, int line, struct params_t *params, const void *data, uint64_t size);
