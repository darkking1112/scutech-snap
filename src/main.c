...
MODULE_VERSION(SCUTECH_SNAP_VERSION);

//printing macros
//#define LOG_DEBUG(fmt, args...) \
	do{ \
		if(scutech_snap_debug) printk(KERN_DEBUG "scutech_snap: " fmt "\n", ## args); \
	}while(0)

#define LOG_DEBUG(fmt, args...) \
	do{ \
		printk(KERN_DEBUG "scutech_snap: " fmt "\n", ## args); \
	}while(0)

#define LOG_WARN(fmt, args...) printk(KERN_WARNING "scutech_snap: " fmt "\n", ## args)
#define LOG_ERROR(error, fmt, args...) printk(KERN_ERR "scutech_snap: " fmt ": %d\n", ## args, error)
...
#define scutech_snap_get_dentry(f) (f)->f_dentry
#define scutech_snap_get_mnt(f) (f)->f_vfsmnt
#else
#define scutech_snap_get_dentry(f) (f)->f_path.dentry
#define scutech_snap_get_mnt(f) (f)->f_path.mnt
#endif

#ifdef HAVE_BDEVNAME
#define scutech_snap_bdevname(dev, buf) bdevname(dev, buf)
#else
#define scutech_snap_bdevname(dev, buf) snprintf(buf, sizeof(buf), "%pg", dev);
#endif
...
#define scutech_snap_d_path(path, page_buf, page_size) d_path((path)->dentry, (path)->mnt, page_buf, page_size)
#define scutech_snap_get_nd_dentry(nd) (nd).dentry
#define scutech_snap_get_nd_mnt(nd) (nd).mnt
#else
#define scutech_snap_d_path(path, page_buf, page_size) d_path(path, page_buf, page_size)
#define scutech_snap_get_nd_dentry(nd) (nd).path.dentry
#define scutech_snap_get_nd_mnt(nd) (nd).path.mnt
#endif
...
struct super_block* (*scutech_snap_get_super)(struct block_device *) = (GET_SUPER_ADDR != 0) ?
...
struct block_device *scutech_snap_lookup_bdev(const char *pathname, fmode_t mode) {
	int r;
	...
	inode = scutech_snap_get_nd_dentry(nd)->d_inode;
	if (!inode) {
		r = -ENOENT;
		goto fail;
	}
...
static struct block_device *blkdev_get_by_path(const char *path, fmode_t mode, void *holder){
	...
	bdev = scutech_snap_lookup_bdev(path, mode);
	if(IS_ERR(bdev))
		return bdev;
...
static inline void scutech_snap_set_bio_ops(struct bio *bio, req_op_t op, unsigned op_flags){
	bio->bi_rw = 0;
	...
	#define scutech_snap_set_bio_ops(bio, op, flags) bio_set_op_attrs(bio, op, flags)
#endif

	#define bio_is_discard(bio) ((bio)->bi_rw & REQ_DISCARD)
	#define scutech_snap_submit_bio(bio) (0, bio)
	#define scutech_snap_submit_bio_wait(bio) submit_bio_wait(0, bio)

static inline int scutech_snap_bio_op_flagged(struct bio *bio, unsigned int flag){
	return bio->bi_rw & flag;
}

static inline void scutech_snap_bio_op_set_flag(struct bio *bio, unsigned int flag){
	bio->bi_rw |= flag;
}

static inline void scutech_snap_bio_op_clear_flag(struct bio *bio, unsigned int flag){
	bio->bi_rw &= ~flag;
}
#else
...
static inline void scutech_snap_bio_op_clear_flag(struct bio *bio, unsigned int flag){
	...
	#endif
	#define scutech_snap_submit_bio(bio) submit_bio(bio)
	#define scutech_snap_submit_bio_wait(bio) submit_bio_wait(bio)
#endif
...
#ifdef HAVE_BIO_ENDIO_INT
static void scutech_snap_bio_endio(struct bio *bio, int err){
...
#define scutech_snap_bdev_stack_limits(queue, bdev, start) bdev_stack_limits(&(queue)->limits, bdev, start)
...
static int kern_path(const char *name, unsigned int flags, struct path *path){
	...
	if(!ret){
		path->dentry = scutech_snap_get_nd_dentry(nd);
		path->mnt = scutech_snap_get_nd_mnt(nd);
	}
	return ret;
}
#endif
...
#define scutech_snap_bioset_create(bio_size, bvec_size, scale) bioset_create(bio_size, bvec_size, BIOSET_NEED_BVECS)
#elif defined HAVE_BIOSET_CREATE_3
#define scutech_snap_bioset_create(bio_size, bvec_size, scale) bioset_create(bio_size, bvec_size, scale)
#else
#define scutech_snap_bioset_create(bio_size, bvec_size, scale) bioset_create(bio_size, scale)
#endif
...
static int scutech_snap_should_remove_suid(struct dentry *dentry)
{
	...
	#define scutech_snap_bio_bi_disk(bio) ((bio)->bi_bdev->bd_disk)
#else
//#if LINUX_VERSION_CODE < KERNEL_VERSION(5,12,0)
	#define scutech_snap_bio_bi_disk(bio) ((bio)->bi_disk)
#endif

#ifdef HAVE_BLKDEV_PUT_1
//#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,28)
	#define scutech_snap_blkdev_put(bdev) blkdev_put(bdev);
#else
	#define scutech_snap_blkdev_put(bdev) blkdev_put(bdev, FMODE_READ);
#endif

#ifdef HAVE_BDEV_NR_SECTORS
	#define scutech_snap_bdev_size(bdev) bdev_nr_sectors(bdev)
#elif defined HAVE_PART_NR_SECTS_READ
//#if LINUX_VERSION_CODE < KERNEL_VERSION(5,10,0)
	#define scutech_snap_bdev_size(bdev) part_nr_sects_read((bdev)->bd_part)
#else
//#if LINUX_VERSION_CODE < KERNEL_VERSION(3,6,0)
	#define scutech_snap_bdev_size(bdev) ((bdev)->bd_part->nr_sects)
#endif

#ifndef HAVE_BDEV_IS_PARTITION
//#if LINUX_VERSION_CODE < KERNEL_VERSION(5,10,0)
	#define scutech_snap_bdev_is_partition(bdev) (bdev->bd_contains != bdev)
#else
	#define scutech_snap_bdev_is_partition(bdev) bdev_is_partition(bdev)
#endif
...
static inline make_request_fn* scutech_snap_get_bd_mrf(struct block_device *bdev){
	return bdev->bd_disk->queue->make_request_fn;
}

static inline void scutech_snap_set_bd_mrf(struct block_device *bdev, make_request_fn *mrf){
	bdev->bd_disk->queue->make_request_fn = mrf;
}

#else
static inline struct block_device_operations* scutech_snap_get_bd_ops(struct block_device *bdev){
	return (struct block_device_operations*)bdev->bd_disk->fops;
}

static inline void scutech_snap_set_bd_ops(struct block_device *bdev, const struct block_device_operations *bd_ops){
...
static inline struct request_queue *scutech_snap_bio_get_queue(struct bio *bio);

#ifdef HAVE_MAKE_REQUEST_FN_INT
	#define MRF_RETURN_TYPE int
	#define MRF_RETURN(ret) return ret

static inline int __scutech_snap_call_mrf(make_request_fn *fn, struct request_queue *q, struct bio *bio){
	return fn(q, bio);
}

static inline int scutech_snap_call_mrf(make_request_fn *fn, struct bio *bio){
	return __scutech_snap_call_mrf(fn, scutech_snap_bio_get_queue(bio), bio);
}
...
static inline int scutech_snap_call_mrf(make_request_fn *fn, struct bio *bio){
	__scutech_snap_call_mrf(fn, scutech_snap_bio_get_queue(bio), bio);
	return 0;
}
#else
...
// Linux version 5.8
static inline MRF_RETURN_TYPE scutech_snap_null_mrf(struct request_queue *q, struct bio *bio){
...
static inline MRF_RETURN_TYPE scutech_snap_null_mrf(struct bio *bio){
#ifndef MRF_RETURN_TYPE_VOID
	percpu_ref_get(&scutech_snap_bio_bi_disk(bio)->queue->q_usage_counter);
#endif
...
static inline void scutech_snap_inode_lock(struct inode *inode){
	mutex_lock(&inode->i_mutex);
}

static inline void scutech_snap_inode_unlock(struct inode *inode){
	mutex_unlock(&inode->i_mutex);
}
#else
	#define scutech_snap_inode_lock inode_lock
	#define scutech_snap_inode_unlock inode_unlock
#endif
...
static inline void file_switch_lock(struct file *filp, bool lock, bool mark_dirty)
{
	...
	inode = scutech_snap_get_dentry(filp)->d_inode;
	igrab(inode);
...
#else
	return scutech_snap_bio_bi_disk(bio)->queue;
#endif
}

static inline void scutech_snap_bio_set_dev(struct bio *bio, struct block_device *bdev){
...
static inline void scutech_snap_bio_copy_dev(struct bio *dst, struct bio *src){
...
//name macros
#define INFO_PROC_FILE "scutech-snap-info"
#define DRIVER_NAME "scutech-snap"
#define CONTROL_DEVICE_NAME "scutech-snap-ctl"
#define SNAP_DEVICE_NAME "scutech-snap%d"
#define SNAP_COW_THREAD_NAME_FMT "scutech_snap_cow%d"
#define SNAP_MRF_THREAD_NAME_FMT "scutech_snap_mrf%d"
#define INC_THREAD_NAME_FMT "scutech_snap_inc%d"
...
#define tracer_for_each_full(dev, i) for(i = 0, dev = ACCESS_ONCE(snap_devices[i]); i < scutech_snap_max_snap_devices; i++, dev = ACCESS_ONCE(snap_devices[i]))

#ifdef USE_BDOPS_SUBMIT_BIO
//returns true if tracing struct's base device fops matches that of bio
#define tracer_matches_bio(dev, bio) (scutech_snap_get_bd_ops(dev->sd_base_dev) == scutech_snap_bio_bi_disk(bio)->fops)
#else
//returns true if tracing struct's base device queue matches that of bio
#define tracer_matches_bio(dev, bio) (bdev_get_queue((dev)->sd_base_dev) == scutech_snap_bio_get_queue(bio))
#endif
 ...
 */
#define __SCUTECH_SNAP_PASSTHROUGH 28	// set as the last flag bit
#else
// set as an unused flag in versions older than 4.8
// set as an unused opcode bit in kernels newer than 4.9
#define __SCUTECH_SNAP_PASSTHROUGH 30
#endif
#define SCUTECH_SNAP_PASSTHROUGH (1ULL << __SCUTECH_SNAP_PASSTHROUGH)

#define SCUTECH_SNAP_DEFAULT_SNAP_DEVICES 24
#define SCUTECH_SNAP_MAX_SNAP_DEVICES 255
...
static int scutech_snap_may_hook_syscalls = 1;
static unsigned long scutech_snap_cow_ext_buf_size = sizeof(struct fiemap_extent) * 1024;
static unsigned long scutech_snap_cow_max_memory_default = (300 * 1024 * 1024);
static unsigned int scutech_snap_cow_fallocate_percentage_default = 10;
static unsigned int scutech_snap_max_snap_devices = SCUTECH_SNAP_DEFAULT_SNAP_DEVICES;
static int scutech_snap_debug = 0;

module_param_named(may_hook_syscalls, scutech_snap_may_hook_syscalls, int, S_IRUGO);
...
module_param_named(cow_ext_buf_size, scutech_snap_cow_ext_buf_size, ulong, 0);
MODULE_PARM_DESC(cow_ext_buf_size, "length of the cow file extension buffer (in bytes)");

module_param_named(cow_max_memory_default, scutech_snap_cow_max_memory_default, ulong, 0);
MODULE_PARM_DESC(cow_max_memory_default, "default maximum cache size (in bytes)");

module_param_named(cow_fallocate_percentage_default, scutech_snap_cow_fallocate_percentage_default, uint, 0);
MODULE_PARM_DESC(cow_fallocate_percentage_default, "default space allocated to the cow file (as integer percentage)");

module_param_named(max_snap_devices, scutech_snap_max_snap_devices, uint, S_IRUGO);
MODULE_PARM_DESC(max_snap_devices, "maximum number of tracers available");

module_param_named(debug, scutech_snap_debug, int, S_IRUGO | S_IWUSR);
...
static int scutech_snap_proc_show(struct seq_file *m, void *v);
static void *scutech_snap_proc_start(struct seq_file *m, loff_t *pos);
static void *scutech_snap_proc_next(struct seq_file *m, void *v, loff_t *pos);
static void scutech_snap_proc_stop(struct seq_file *m, void *v);
static int scutech_snap_proc_open(struct inode *inode, struct file *filp);
static int scutech_snap_proc_release(struct inode *inode, struct file *file);
...
#define SCUTECH_SNAP_WAIT_FOR_RELEASE_MSEC              500
#define SCUTECH_SNAP_WAIT_FOR_RELEASE_MAX_SLEEP_COUNT   100
static void scutech_snap_wait_for_release(struct snap_device *dev);
...
static const struct seq_operations scutech_snap_seq_proc_ops = {
	.start = scutech_snap_proc_start,
	.next = scutech_snap_proc_next,
	.stop = scutech_snap_proc_stop,
	.show = scutech_snap_proc_show,
};

#ifdef HAVE_PROC_CREATE_FN_PROC_OPS
static const struct proc_ops scutech_snap_proc_fops = {
	.proc_open = scutech_snap_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = scutech_snap_proc_release,
};
#else
static const struct file_operations scutech_snap_proc_fops = {
	.owner = THIS_MODULE,
	.open = scutech_snap_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = scutech_snap_proc_release,
};
#endif
...
static int file_open(const char *filename, int flags, struct file **filp){
	...
	if(!f){
		...
		goto error;
	}else if(!S_ISREG(scutech_snap_get_dentry(f)->d_inode->i_mode)){
		ret = -EINVAL;
...
static int path_get_absolute_pathname(const struct path *path, char **buf, int *len_res){
	...
	pathname = scutech_snap_d_path(path, page_buf, PAGE_SIZE);
	if(IS_ERR(pathname)){
		ret = PTR_ERR(pathname);
		pathname = NULL;
...
static int file_get_absolute_pathname(const struct file *filp, char **buf, int *len_res){
	...
	path.mnt = scutech_snap_get_mnt(filp);
	path.dentry = scutech_snap_get_dentry(filp);
...
write_bio:
	...
	scutech_snap_bio_set_dev(new_bio, bdev);
	scutech_snap_set_bio_ops(new_bio, REQ_OP_WRITE, 0);
	...
	ret = scutech_snap_submit_bio_wait(new_bio);
	if (ret) {
...
read_bio:
	...
	scutech_snap_bio_set_dev(new_bio, bdev);
	scutech_snap_set_bio_ops(new_bio, REQ_OP_READ, 0);
	bio_sector(new_bio) = start_sect;
	bio_idx(new_bio) = 0;
...
static inline ssize_t scutech_snap_kernel_read(struct cow_manager *cm, void *buf, size_t count, loff_t *pos){
	ssize_t ret;
...
static inline ssize_t scutech_snap_kernel_write(struct cow_manager *cm, void *buf, size_t count, loff_t *pos){
	ssize_t ret;
...
static int file_io(struct cow_manager *cm, int is_write, void *buf, sector_t offset, unsigned long len){
	...
	if(is_write) {
        LOG_DEBUG("Writing %lu bytes to offset %llu", len, (unsigned long long)offset);
        ret = scutech_snap_kernel_write(cm, buf, len, &off);
    }
	else {
        LOG_DEBUG("Reading %lu bytes from offset %llu", len, (unsigned long long)offset);
        ret = scutech_snap_kernel_read(cm, buf, len, &off);}
...
static int scutech_snap_do_truncate(struct dentry *dentry, loff_t length, unsigned int time_attrs, struct file *filp){
	int ret;
	...
	ret = scutech_snap_should_remove_suid(dentry);
	if(ret) newattrs.ia_valid |= ret | ATTR_FORCE;

	scutech_snap_inode_lock(dentry->d_inode);
...
#endif
	scutech_snap_inode_unlock(dentry->d_inode);
...
static int file_truncate(struct file *filp, loff_t len){
	...
	dentry = scutech_snap_get_dentry(filp);
...
//#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0)
	...
	ret = scutech_snap_do_truncate(dentry, len, ATTR_MTIME|ATTR_CTIME, filp);
...
static int real_fallocate(struct file *f, uint64_t offset, uint64_t length){
	...
	struct inode *inode = scutech_snap_get_dentry(f)->d_inode;
	#else
...
static int __file_unlink(struct file *filp, int close, int force){
	int ret = 0;
	struct inode *dir_inode = scutech_snap_get_dentry(filp)->d_parent->d_inode;
	struct dentry *file_dentry = scutech_snap_get_dentry(filp);
	struct vfsmount *mnt = scutech_snap_get_mnt(filp);
...
static int __cow_write_section(struct cow_manager *cm, unsigned long sect_idx){
    ...
    LOG_DEBUG("Writing section %lu to file, sect_size_bytes %lu", sect_idx, sect_size_bytes);
...
static int __cow_sync_and_free_sections(struct cow_manager *cm, unsigned long thresh, bool fill_ahead){
    ...
    LOG_DEBUG("Syncing and freeing sections, thresh %lu", thresh);
...
static inline void scutech_snap_mm_lock(struct mm_struct *mm)
{
...
static inline void scutech_snap_mm_unlock(struct mm_struct *mm)
{
#ifdef HAVE_MMAP_WRITE_LOCK
...
static struct vm_area_struct *scutech_snap_vm_area_allocate(struct mm_struct *mm)
{
...
static void scutech_snap_vm_area_free(struct vm_area_struct *vma)
{
	kmem_cache_free(*vm_area_cache, vma);
}

static int scutech_snap_get_cow_file_extents(struct snap_device *dev, struct file *filp)
{
	int ret;
	...
	unsigned long cow_ext_buf_size = ALIGN(scutech_snap_cow_ext_buf_size, PAGE_SIZE);
	...
	vma = scutech_snap_vm_area_allocate(task->mm);
	if (!vma) {
		ret = -ENOMEM;
		LOG_ERROR(ret, "vm_area_alloc() failed");
		scutech_snap_mm_unlock(task->mm);
		return ret;
	}
...
static int cow_sync_and_close(struct cow_manager *cm){
	...
	ret = scutech_snap_get_cow_file_extents(cm->dev, cm->filp);
	if(ret) goto error;
...
static unsigned long __cow_calculate_allowed_sects(unsigned long cache_size, unsigned long total_sects){
    if(cache_size <= (total_sects * sizeof(struct cow_section))) {
        LOG_DEBUG("total_sects = %lu, size = %lu, cache_size = %lu", total_sects, sizeof(struct cow_section), cache_size);
        return 0;}
...
static int cow_init(struct snap_device *dev, const char *path, uint64_t elements, unsigned long sect_size, unsigned long cache_size, uint64_t file_max, const uint8_t *uuid, uint64_t seqid, struct cow_manager **cm_out){
    ...
    LOG_DEBUG("elements = %lu, sect_size = %lu,cm->log_sect_pages = %lu, total_sects = %lu, allowed_sects = %lu, data_offset = %lu, curr_pos = %lu",
               elements, sect_size,cm->log_sect_pages , cm->total_sects, cm->allowed_sects, cm->data_offset, cm->curr_pos);
...
static int __cow_write_mapping(struct cow_manager *cm, uint64_t pos, uint64_t val){
    ...
    LOG_DEBUG("pos = %llu, sect_idx = %llu, sect_pos = %lu", pos, sect_idx, sect_pos);
...
static int __cow_write_data(struct cow_manager *cm, void *buf){
    ...
    LOG_DEBUG("write data to cow file, curr_size = %llu, COW_BLOCK_SIZE = %d", curr_size, COW_BLOCK_SIZE);
	if(ret) goto error;
...
static int tracing_ops_alloc(struct snap_device *dev) {
	...
	memcpy(trops->bd_ops, scutech_snap_get_bd_ops(dev->sd_base_dev), sizeof(struct block_device_operations));
...
static int bio_make_read_clone(struct block_device *bdev, struct bio_set *bs, struct tracing_params *tp, struct bio *orig_bio, sector_t sect, unsigned int pages, struct bio **bio_out, unsigned int *bytes_added){
    ...
    LOG_DEBUG("create new bio: new_bio = %p, actual_pages = %u", new_bio, actual_pages);
	if(!new_bio){
		ret = -ENOMEM;
	...
	scutech_snap_bio_copy_dev(new_bio, orig_bio);
	scutech_snap_set_bio_ops(new_bio, REQ_OP_READ, 0);
...
static int snap_handle_read_bio(const struct snap_device *dev, struct bio *bio){
	...
	scutech_snap_bio_set_dev(bio, dev->sd_base_dev);
	scutech_snap_set_bio_ops(bio, REQ_OP_READ, READ_SYNC);
	...
	if(mode != READ_MODE_COW_FILE){
		...
		ret = scutech_snap_submit_bio_wait(bio);
		if(ret){
...
static int snap_handle_write_bio(const struct snap_device *dev, struct bio *bio){
	...
	bio_for_each_segment_all(bvec, bio, i) {
		...
		data = kmap(bvec->bv_page);
#ifdef NETLINK_DEBUG
	trace_event_cow_data(EVENT_COW_WRITE_DATA, bio, 0, data, bvec->bv_len);
    LOG_DEBUG("log bio = %p, data = %p, bvec->bv_len = %u", bio, data, bvec->bv_len);
#endif
...
static int snap_mrf_thread(void *data){
	...
	while(!kthread_should_stop() || !bio_queue_empty(bq)) {
		...
		scutech_snap_bio_op_set_flag(bio, SCUTECH_SNAP_PASSTHROUGH);
#ifdef NETLINK_DEBUG
		trace_event_bio(EVENT_BIO_CALL_ORIG, bio, 0);
#endif
		ret = scutech_snap_call_mrf(dev->sd_orig_mrf, bio);
        LOG_DEBUG("mrf thread scutech_snap_call_mrf returned %d", ret);
#ifdef HAVE_MAKE_REQUEST_FN_INT
	...
	while(!kthread_should_stop() || !bio_queue_empty(bq) || atomic64_read(&dev->sd_submitted_cnt) != atomic64_read(&dev->sd_received_cnt)) {
		...
		*/
		...
		if(!bio_data_dir(bio)){
			...
			if(is_failed){
				scutech_snap_bio_endio(bio, wrap_err_io(dev)); //end the bio with an IO error
				continue;
			}
            LOG_DEBUG("snap_handle_read_bio: bio = %p", bio);
			...
			scutech_snap_bio_endio(bio, (ret)? wrap_err_io(dev) : 0);
		}else{
			if(is_failed){
				bio_free_clone(bio);
				continue;
			...
			{
                LOG_DEBUG("snap_handle_write_bio: bio = %p", bio);
...
static void __on_bio_read_complete(struct bio *bio, int err){
    ...
    LOG_DEBUG("__on_bio_read_complete bio = %p, err = %d", bio, err);
	...
	scutech_snap_set_bio_ops(bio, REQ_OP_WRITE, 0);
...
//#if LINUX_VERSION_CODE < KERNEL_VERSION(3,14,0)
    ...
    LOG_DEBUG("bio_queue_add: bio = %p, receive_cnt = %d.", bio, dev->sd_received_cnt);
	smp_wmb();
...
static int snap_trace_bio(struct snap_device *dev, struct bio *bio){
	...
	if (!bio_needs_cow(bio, dev) || memory_is_too_low(dev) || tracer_read_fail_state(dev)) {
		...
		trace_event_bio(EVENT_BIO_CALL_ORIG, bio, 0);
#endif
		return scutech_snap_call_mrf(dev->sd_orig_mrf, bio);
	}
    ...
    LOG_DEBUG("start_sect = %lu, end_sect = %lu, pages = %u", start_sect, end_sect, pages);
...
retry:
    ...
    LOG_DEBUG("Submitted BIO count: %lld", dev->sd_submitted_cnt);
	smp_wmb();
	...
	if (dev->sd_orig_mrf) {
        LOG_DEBUG("scutech_snap_call_mrf(dev->sd_orig_mrf, new_bio)");
		scutech_snap_call_mrf(dev->sd_orig_mrf, new_bio);
	} else {
        // call submit_bio() directly
        LOG_DEBUG("call submit_bio() directly");
		scutech_snap_submit_bio(new_bio);
	}
#else
	scutech_snap_submit_bio(new_bio);
#endif
...
static MRF_RETURN_TYPE tracing_mrf(struct request_queue *q, struct bio *bio){
	...
	tracer_for_each(dev, i){	// for each snap device
		if(!dev || test_bit(UNVERIFIED, &dev->sd_state) || !tracer_matches_bio(dev, bio))
            continue;

		orig_mrf = dev->sd_orig_mrf;
		if(scutech_snap_bio_op_flagged(bio, SCUTECH_SNAP_PASSTHROUGH)){
#ifdef NETLINK_DEBUG
			trace_event_bio(EVENT_BIO_CALL_ORIG, bio, 0);
#endif

			scutech_snap_bio_op_clear_flag(bio, SCUTECH_SNAP_PASSTHROUGH);
			goto call_orig;
		}
...
call_orig:
	...
	// Linux version 5.9+
	if (orig_mrf) {
		ret = scutech_snap_call_mrf(orig_mrf, bio);
	} else if (scutech_snap_bio_bi_disk(bio)->fops->submit_bio) {
		if (scutech_snap_bio_bi_disk(bio)->fops->submit_bio == tracing_mrf) {
			MRF_SET_RETURN_VALUE(scutech_snap_null_mrf(bio));
		} else {
			MRF_SET_RETURN_VALUE(scutech_snap_bio_bi_disk(bio)->fops->submit_bio(bio));
		}
	} else {
...
#else
	if(orig_mrf) {
		ret = __scutech_snap_call_mrf(orig_mrf, q, bio);
	} else {
		LOG_WARN("error finding original_mrf");
		MRF_SET_RETURN_VALUE(scutech_snap_submit_bio(bio));
	}
#endif
out:
	MRF_RETURN(ret);
...
// Linux version >= 5.9
static MRF_RETURN_TYPE snap_mrf(struct bio *bio){
	struct snap_device *dev = scutech_snap_bio_bi_disk(bio)->queue->queuedata;
#endif
	...
	if(bio_data_dir(bio)){
		scutech_snap_bio_endio(bio, -EOPNOTSUPP);
		MRF_RETURN(0);
	}else if(tracer_read_fail_state(dev)){
		scutech_snap_bio_endio(bio, wrap_err_io(dev));
		MRF_RETURN(0);
	}else if(!test_bit(ACTIVE, &dev->sd_state)){
		scutech_snap_bio_endio(bio, -EBUSY);
		MRF_RETURN(0);
	}
...
static int find_orig_mrf(struct block_device *bdev, make_request_fn **mrf){
	...
	make_request_fn *orig_mrf = scutech_snap_get_bd_mrf(bdev);

	if(orig_mrf != tracing_mrf){
#ifdef HAVE_BLK_MQ_MAKE_REQUEST
		// Linux version 5.8
		if (!orig_mrf){
			orig_mrf = scutech_snap_null_mrf;
			LOG_DEBUG("original mrf is empty, set to scutech_snap_null_mrf");
		}
#endif
		*mrf = orig_mrf;
		return 0;
...
static int find_orig_fops(struct block_device *bdev, struct block_device_operations **ops, make_request_fn **mrf, struct tracing_ops **tracing_ops){
	int i;
	struct snap_device *dev;
	struct block_device_operations *orig_ops = scutech_snap_get_bd_ops(bdev);
	...
	*tracing_ops = NULL;
	...
	if(orig_mrf != tracing_mrf){
		if (!orig_mrf){
			if (!elastio_blk_mq_submit_bio){
				LOG_ERROR(-EFAULT, "error finding original mrf, original submit_bio and scutech_snap_null_mrf both are empty");
				return -EFAULT;
			}

			orig_mrf = scutech_snap_null_mrf;
			LOG_DEBUG("original mrf is empty, set to scutech_snap_null_mrf = %p; orig ops = %p", orig_mrf, orig_ops);
		}
		else {
	...
	tracer_for_each(dev, i){
		if(!dev || test_bit(UNVERIFIED, &dev->sd_state)) continue;
		if(orig_ops == scutech_snap_get_bd_ops(dev->sd_base_dev)){
			*ops = dev->sd_orig_ops;
			*mrf = dev->sd_orig_mrf;
			*tracing_ops = tracing_ops_get(dev->sd_tracing_ops);
			scutech_snap_bdevname(dev->sd_base_dev, bdev_name);
...
#else
	struct block_device_operations *ops = scutech_snap_get_bd_ops(dev->sd_base_dev);
#endif

	if(scutech_snap_get_bd_mrf(dev->sd_base_dev) != tracing_mrf) return 0;
...
#else
			if(ops == scutech_snap_get_bd_ops(cur_dev->sd_base_dev)) return 0;
#endif
		}
	}
...
static int __tracer_transition_tracing(struct snap_device *dev, struct block_device *bdev, const struct block_device_operations *new_ops, struct snap_device **dev_ptr, bool start){
#endif
	int ret;
	struct super_block *origsb = scutech_snap_get_super(bdev);
	...
	scutech_snap_bdevname(bdev, bdev_name);
	...
	if(start){
		...
		if(new_ops) scutech_snap_set_bd_ops(bdev, new_ops);
#ifdef HAVE_BD_HAS_SUBMIT_BIO
		bdev->bd_has_submit_bio = true; // kernel version >= 6.4
#endif
#else
		if(new_mrf) scutech_snap_set_bd_mrf(bdev, new_mrf);
#endif
#ifdef NETLINK_DEBUG
...
#ifdef HAVE_BLK_MQ_MAKE_REQUEST
// Linux version 5.8
		if(new_mrf) scutech_snap_set_bd_mrf(bdev, new_mrf == scutech_snap_null_mrf ? NULL : new_mrf);
...
static void __tracer_destroy_base_dev(struct snap_device *dev){
	...
	if(dev->sd_base_dev){
		...
		scutech_snap_blkdev_put(dev->sd_base_dev);
...
static int __tracer_setup_base_dev(struct snap_device *dev, const char *bdev_path){
	...
	if(scutech_snap_bdev_is_partition(dev->sd_base_dev)){
		dev->sd_sect_off = get_start_sect(dev->sd_base_dev);
		dev->sd_size = scutech_snap_bdev_size(dev->sd_base_dev);
	}else{
		dev->sd_sect_off = 0;
...
static int file_is_on_bdev(const struct file *file, struct block_device *bdev) {
	struct super_block *sb = scutech_snap_get_super(bdev);
	int ret = 0;
	if (sb) {
		ret = ((scutech_snap_get_mnt(file))->mnt_sb == sb);
		drop_super(sb);
	}
	return ret;
}
		...
		sector_t size, unsigned long fallocated_space, unsigned long cache_size, const uint8_t *uuid, uint64_t seqid, int open_method)
{
	...
	if(open_method == 3){
		...
		if(!cache_size) dev->sd_cache_size = scutech_snap_cow_max_memory_default;
		else dev->sd_cache_size = cache_size;

		if(open_method == 0){
			//calculate how much space should be allocated to the cow file
			if(!fallocated_space){
				max_file_size = size * SECTOR_SIZE * scutech_snap_cow_fallocate_percentage_default;
	...
	if (file_is_on_bdev(dev->sd_cow->filp, bdev)) {
		...
		dev->sd_cow_inode = scutech_snap_get_dentry(dev->sd_cow->filp)->d_inode;
	}
...
static int __tracer_setup_cow_path(struct snap_device *dev, const struct file *cow_file){
	...
	if(test_bit(COW_ON_BDEV, &dev->sd_cow_state)){
		...
		ret = dentry_get_relative_pathname(scutech_snap_get_dentry(cow_file), &dev->sd_cow_path, NULL);
	}else{
...
#ifndef HAVE_BIOSET_INIT
//#if LINUX_VERSION_CODE < KERNEL_VERSION(4,18,0)
	dev->sd_bioset = scutech_snap_bioset_create(BIO_SET_SIZE, BIO_SET_SIZE, 0);
...
// alloc_disk() function has disappeared starting from the kernel 5.15
	...
	scutech_snap_bdev_stack_limits(dev->sd_queue, bdev, 0);
...
static void minor_range_recalculate(void){
	unsigned int i, highest = 0, lowest = scutech_snap_max_snap_devices - 1;
	struct snap_device *dev;
...
static void tracer_reconfigure(struct snap_device *dev, unsigned long cache_size){
	dev->sd_cache_size = cache_size;
	if(!cache_size) cache_size = scutech_snap_cow_max_memory_default;
	if(test_bit(ACTIVE, &dev->sd_state)) cow_modify_cache_size(dev->sd_cow, cache_size);
}

static void tracer_scutech_snap_info(const struct snap_device *dev, struct scutech_snap_info *info){
	info->minor = dev->sd_minor;
	info->state = dev->sd_state;
	info->error = tracer_read_fail_state(dev);
	info->cache_size = (dev->sd_cache_size)? dev->sd_cache_size : scutech_snap_cow_max_memory_default;
...
static int __verify_minor(unsigned int minor, int mode){
	...
	if(minor >= scutech_snap_max_snap_devices){
...
static int __verify_bdev_writable(const char *bdev_path, int *out){
	...
	sb = scutech_snap_get_super(bdev);
	if(sb){
		writable = !(sb->s_flags & MS_RDONLY);
		drop_super(sb);
	}

	scutech_snap_blkdev_put(bdev);
	*out = writable;
	return 0;
}
...
static int ioctl_scutech_snap_info(struct scutech_snap_info *info){
	int ret;
	struct snap_device *dev;

	LOG_DEBUG("received scutech-snap info ioctl - %u", info->minor);
	...
	tracer_scutech_snap_info(dev, info);
...
static long ctrl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg){
	...
	char *bdev_path = NULL;
	char *cow_path = NULL;
	struct scutech_snap_info *info = NULL;
	...
	case IOCTL_SETUP_SNAP:
		...
		scutech_snap_wait_for_release(snap_devices[minor]);
	...
	case IOCTL_RECONFIGURE:
		...
		break;
	case IOCTL_SCUTECH_SNAP_INFO:
		//get params from user space
		info = kmalloc(sizeof(struct scutech_snap_info), GFP_KERNEL);
		if(!info){
			ret = -ENOMEM;
			LOG_ERROR(ret, "error allocating memory for scutech-snap-info");
			break;
		}

		ret = copy_from_user(info, (struct scutech_snap_info __user *)arg, sizeof(struct scutech_snap_info));
		if(ret){
			ret = -EFAULT;
			LOG_ERROR(ret, "error copying scutech-snap-info struct from user space");
			break;
		}

		ret = ioctl_scutech_snap_info(info);
		if(ret) break;

		ret = copy_to_user((struct scutech_snap_info __user *)arg, info, sizeof(struct scutech_snap_info));
		if(ret){
			ret = -EFAULT;
			LOG_ERROR(ret, "error copying scutech-snap-info struct to user space");
			break;
		}
...
static int __handle_bdev_mount_nowrite(const struct vfsmount *mnt, unsigned int *idx_out){
	...
	tracer_for_each(dev, i){
		...
		if (mnt->mnt_root != scutech_snap_get_mnt(dev->sd_cow->filp)->mnt_root)
			continue;
	...
	tracer_for_each(dev, i){
		...
		if(test_bit(UNVERIFIED, &dev->sd_state)){
			...
			if(cur_bdev == bdev){
				...
				scutech_snap_blkdev_put(cur_bdev);
...
static void post_umount_check(int dormant_ret, long umount_ret, unsigned int idx, const char __user *dir_name){
	...
	sb = scutech_snap_get_super(dev->sd_base_dev);
	if(sb){
...
static int scutech_snap_proc_show(struct seq_file *m, void *v){
	...
	if(dev_ptr != SEQ_START_TOKEN && *dev_ptr != NULL){
		...
		seq_printf(m, "\t\t\t\"max_cache\": %lu,\n", (dev->sd_cache_size)? dev->sd_cache_size : scutech_snap_cow_max_memory_default);
...
static inline void *scutech_snap_proc_get_idx(loff_t pos){
...
static void *scutech_snap_proc_start(struct seq_file *m, loff_t *pos){
	...
	return scutech_snap_proc_get_idx(*pos - 1);
}

static void *scutech_snap_proc_next(struct seq_file *m, void *v, loff_t *pos){
	void *dev = scutech_snap_proc_get_idx(*pos);
	++*pos;
	return dev;
}
...
static int scutech_snap_proc_open(struct inode *inode, struct file *filp){
	mutex_lock(&ioctl_mutex);
	return seq_open(filp, &scutech_snap_seq_proc_ops);
}
...
static void scutech_snap_wait_for_release(struct snap_device *dev)
{
	...
	while (atomic_read(&dev->sd_refs) && i < SCUTECH_SNAP_WAIT_FOR_RELEASE_MAX_SLEEP_COUNT) {
		msleep(SCUTECH_SNAP_WAIT_FOR_RELEASE_MSEC / SCUTECH_SNAP_WAIT_FOR_RELEASE_MAX_SLEEP_COUNT);
		++i;
	}
...
static int __init agent_init(void){
	...
	//init minor range
	if(scutech_snap_max_snap_devices == 0 || scutech_snap_max_snap_devices > SCUTECH_SNAP_MAX_SNAP_DEVICES){
		const unsigned int nr_devices = scutech_snap_max_snap_devices == 0 ? SCUTECH_SNAP_DEFAULT_SNAP_DEVICES : SCUTECH_SNAP_MAX_SNAP_DEVICES;
		LOG_WARN("invalid number of snapshot devices (%u), setting to %u", scutech_snap_max_snap_devices, nr_devices);
		scutech_snap_max_snap_devices = nr_devices;
	}

	highest_minor = 0;
	lowest_minor = scutech_snap_max_snap_devices - 1;
	...
	snap_devices = kzalloc(scutech_snap_max_snap_devices * sizeof(struct snap_device*), GFP_KERNEL);
	if(!snap_devices){
		ret = -ENOMEM;
	...
	info_proc = proc_create(INFO_PROC_FILE, 0, NULL, &scutech_snap_proc_fops);
	if(!info_proc){
		ret = -ENOENT;
	...
	if (scutech_snap_may_hook_syscalls) {
