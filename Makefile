...
pkgprep: pkgclean
	...
	tar --exclude=./pkgbuild --exclude=.git --transform 's,^\.,scutech-snap,' -czf $(BUILDDIR)/SOURCES/scutech-snap.tar.gz .
	cp dist/scutech-snap.spec $(BUILDDIR)/SPECS/scutech-snap.spec

deb: check_root pkgprep
	debbuild $(PKGBUILDFLAGS) $(BUILDDIR)/SPECS/scutech-snap.spec

rpm: check_root pkgprep
	rpmbuild $(PKGBUILDFLAGS) $(BUILDDIR)/SPECS/scutech-snap.spec
