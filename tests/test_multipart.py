...
class TestMultipart(DeviceTestCaseMultipart):
    def setUp(self):
        ...
        for i in range(self.part_count):
            ...
            self.snap_devices.append("/dev/scutech-snap{}".format(self.minors[i]))
    ...
    def test_multipart_setup_volumes_same_disk(self):
        ...
        for i in range(self.part_count):
            self.assertEqual(scutech_snap.setup(self.minors[i], self.devices[i], self.cow_full_paths[i]), 0)

            self.assertTrue(os.path.exists(self.snap_devices[i]))

            snapdev = scutech_snap.info(self.minors[i])
        ...
        for i in reversed(range(self.part_count)):
            self.assertEqual(scutech_snap.destroy(self.minors[i]), 0)
    ...
    def test_multipart_setup_volumes_write_last_after_destroy(self):
        ...
        self.assertEqual(scutech_snap.destroy(self.minors[-1]), 0)
    ...
    def test_multipart_modify_origins(self):
        for i in range(self.part_count):
            ...
            self.assertEqual(scutech_snap.setup(self.minors[i], self.devices[i], self.cow_full_paths[i]), 0)
            self.addCleanup(scutech_snap.destroy, self.minors[i])
