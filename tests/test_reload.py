...
class TestReload(DeviceTestCase):
    ...
    def test_reload_snap_invalid_minor(self):
        self.assertEqual(scutech_snap.reload_snapshot(1000, self.device, self.cow_reload_path), errno.EINVAL)
    ...
    def test_reload_inc_invalid_minor(self):
        self.assertEqual(scutech_snap.reload_incremental(1000, self.device, self.cow_reload_path), errno.EINVAL)
        self.assertFalse(os.path.exists(self.snap_device))
        self.assertIsNone(scutech_snap.info(self.minor))

    def test_reload_snap_volume_path_is_dir(self):
        self.assertEqual(scutech_snap.reload_snapshot(self.minor, self.mount, self.cow_full_path), errno.ENOTBLK)
        self.assertFalse(os.path.exists(self.snap_device))
        self.assertIsNone(scutech_snap.info(self.minor))

    def test_reload_inc_volume_path_is_dir(self):
        self.assertEqual(scutech_snap.reload_incremental(self.minor, self.mount, self.cow_full_path), errno.ENOTBLK)
        self.assertFalse(os.path.exists(self.snap_device))
        self.assertIsNone(scutech_snap.info(self.minor))

    def test_reload_snap_device_no_exists(self):
        self.assertEqual(scutech_snap.reload_snapshot(self.minor, "/dev/not_exist_device", self.cow_full_path), errno.ENOENT)
        self.assertFalse(os.path.exists(self.snap_device))
        self.assertIsNone(scutech_snap.info(self.minor))

    def test_reload_inc_device_no_exists(self):
        self.assertEqual(scutech_snap.reload_incremental(self.minor, "/dev/not_exist_device", self.cow_full_path), errno.ENOENT)
        self.assertFalse(os.path.exists(self.snap_device))
        self.assertIsNone(scutech_snap.info(self.minor))
    ...
    def test_reload_unverified_snapshot(self):
        ...
        self.assertEqual(scutech_snap.reload_snapshot(self.minor, self.device, self.cow_reload_path, ignore_snap_errors=True), 0)
        self.addCleanup(scutech_snap.destroy, self.minor)
        self.addCleanup(util.mount, self.device, self.mount)
        self.assertFalse(os.path.exists(self.snap_device))

        self.check_snap_info(0, scutech_snap.State.UNVERIFIED | scutech_snap.State.SNAPSHOT, True)
        ...
        self.check_snap_info(-errno.ENOENT, scutech_snap.State.UNVERIFIED | scutech_snap.State.SNAPSHOT, True)


    def test_reload_unverified_incremental(self):
        util.unmount(self.mount)

        self.assertEqual(scutech_snap.reload_incremental(self.minor, self.device, self.cow_reload_path, ignore_snap_errors=True), 0)
        self.addCleanup(scutech_snap.destroy, self.minor)
        self.addCleanup(util.mount, self.device, self.mount)
        self.assertFalse(os.path.exists(self.snap_device))

        self.check_snap_info(0, scutech_snap.State.UNVERIFIED, True)

        # Mount and test that the non-existent cow file been handled
        util.mount(self.device, self.mount)
        self.addCleanup(util.unmount, self.device, self.mount)

        self.check_snap_info(-errno.ENOENT, scutech_snap.State.UNVERIFIED, True)


    def test_reload_verified_snapshot(self):
        self.assertEqual(scutech_snap.setup(self.minor, self.device, self.cow_full_path, ignore_snap_errors=True), 0)
        ...
        self.check_snap_info(0, scutech_snap.State.UNVERIFIED | scutech_snap.State.SNAPSHOT, False)
        ...
        self.check_snap_info(0, scutech_snap.State.ACTIVE | scutech_snap.State.SNAPSHOT, False, 1)
    ...
    def test_reload_verified_inc(self):
        ...
        self.check_snap_info(0, scutech_snap.State.UNVERIFIED, False)
        ...
        self.check_snap_info(0, scutech_snap.State.ACTIVE, False, 1)
    ...
    def check_snap_info(self, error, state, ignore_snap_errors, version = 0):
        ...
        if  state & scutech_snap.State.ACTIVE:
