...
class TestSetup(DeviceTestCase):
    ...
    def test_setup_invalid_minor(self):
        self.assertEqual(scutech_snap.setup(1000, self.device, self.cow_full_path), errno.EINVAL)
    ...
    def test_setup_volume_path_is_dir(self):
        self.assertEqual(scutech_snap.setup(self.minor, self.mount, self.cow_full_path), errno.ENOTBLK)
        self.assertFalse(os.path.exists(self.snap_device))
        self.assertIsNone(scutech_snap.info(self.minor))

    def test_setup_cow_file_path_is_dir(self):
        self.assertEqual(scutech_snap.setup(self.minor, self.device, self.mount), errno.EISDIR)
        self.assertFalse(os.path.exists(self.snap_device))
        self.assertIsNone(scutech_snap.info(self.minor))
    ...
    def test_setup_unmounted_volume(self):
        ...
        self.assertEqual(scutech_snap.setup(self.minor, self.device, self.cow_full_path), errno.EINVAL)
        self.assertFalse(os.path.exists(self.snap_device))
        self.assertIsNone(scutech_snap.info(self.minor))
    ...
    def test_setup_already_tracked_volume(self):
        ...
        self.assertEqual(scutech_snap.setup(self.minor, self.device, self.cow_full_path), errno.EBUSY)
        self.assertTrue(os.path.exists(self.snap_device))
        self.assertIsNotNone(scutech_snap.info(self.minor))

    def test_setup_check_cow_size_zero_fallocate(self):
        self.assertEqual(scutech_snap.setup(self.minor, self.device, self.cow_full_path, fallocated_space=0), 0)
        self.addCleanup(scutech_snap.destroy, self.minor)
    ...
    def test_setup_check_cow_size_fallocate(self):
        self.assertEqual(scutech_snap.setup(self.minor, self.device, self.cow_full_path, fallocated_space=50), 0)
        self.addCleanup(scutech_snap.destroy, self.minor)
    ...
    def test_setup_volume(self):
        ...
        self.assertEqual(snapdev["state"], scutech_snap.State.ACTIVE | scutech_snap.State.SNAPSHOT)
        ...
        self.assertEqual(snapdev["flags"], scutech_snap.Flags.COW_ON_BDEV)
    ...
    def test_setup_2_volumes(self):
        ...
        snap_device = "/dev/scutech-snap{}".format(minor)
        cow_file = "cow"

        self.assertEqual(scutech_snap.setup(minor, device, "/{}".format(cow_file)), 0)

        # Check the snapshot device exists and alive
        self.assertTrue(os.path.exists(snap_device))

        snapdev = scutech_snap.info(minor)
        self.assertIsNotNone(snapdev)
        ...
        self.assertEqual(scutech_snap.destroy(minor), 0)
