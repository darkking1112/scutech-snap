...
static int handle_setup_snap(int argc, char **argv){
	...
	return scutech_snap_setup_snapshot(minor, bdev, cow, fallocated_space, cache_size, ignore_snap_errors);
...
static int handle_reload_snap(int argc, char **argv){
	...
	return scutech_snap_reload_snapshot(minor, bdev, cow, cache_size, ignore_snap_errors);
...
static int handle_reload_inc(int argc, char **argv){
	...
	return scutech_snap_reload_incremental(minor, bdev, cow, cache_size, ignore_snap_errors);
...
static int handle_destroy(int argc, char **argv){
	...
	return scutech_snap_destroy(minor);
...
static int handle_transition_inc(int argc, char **argv){
	...
	return scutech_snap_transition_incremental(minor);
...
static int handle_transition_snap(int argc, char **argv){
	...
	return scutech_snap_transition_snapshot(minor, cow, fallocated_space);
...
static int handle_reconfigure(int argc, char **argv){
	...
	return scutech_snap_reconfigure(minor, cache_size);
...
static int handle_info(int argc, char **argv){
	...
	ret = scutech_snap_info(minor, &info);
	if(ret == 0) {
		printf("{\n");
		printf("\t\"minor\": %u,\n", info.minor);
		printf("\t\"cow_file\": \"%s\",\n", info.cow);
...
static int handle_get_free_minor(int argc){
	...
	minor = scutech_snap_get_free_minor();
	if(minor < 0) {
		return minor;
	}
...
int main(int argc, char **argv){
	...
	if(access("/dev/scutech-snap-ctl", F_OK) != 0){
		errno = EINVAL;
